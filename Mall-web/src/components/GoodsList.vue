      ...
      <div class="goods-box">
        ...
        <div class="goods-list-box">
          <div class="goods-list-tool">
            <ul>
              <li v-for="(item,index) in goodsTool" :key="index" @click="orderBy(index)">
          ...
          <div class="goods-list">
            <div class="goods-show-info" v-for="(item, index) in orderGoodsList" :key="index">
              <div class="goods-show-img">
                <router-link :to="{path:'/goodsDetail',query:{productId:item.id}}"><img :src="item.img"/></router-link>
              </div>
              ...
              <div class="goods-show-sale">
                <span>{{item.sale}}</span>人已付款
              </div>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        goodsTool: [
          {title: '销量', en: 'sale'},
          {title: '评论数', en: 'comment'},
          {title: '价格', en: 'lowestPrice'}
        ],
        tempSort: {
          'title': 'sale',
          'sortBy': 'ASC'
    ...
    methods: {
      ...
      ...mapMutations(['SET_GOODS_ORDER_SORT_BY']),
      orderBy(index) {
        this.icon = [];
        this.isActio = [false, false, false];
        let data = this.goodsTool[index];
        if (this.tempSort.title === data) {
          // 对相同排序规则更改升序或降序
          if (this.tempSort.sortBy === 'ASC') {
            this.tempSort.sortBy = 'DESC';
            this.icon[index] = 'arrow-down-a';
          } else if (this.tempSort.sortBy === 'DESC') {
            this.tempSort.sortBy = 'ASC';
            this.icon[index] = 'arrow-up-a';
          }
        } else {
          // 改变排序规则
          this.tempSort.title = data;
          this.tempSort.sortBy = 'ASC';
          this.icon[index] = 'arrow-up-a';
          this.isAction = [false, false, false];
          this.isAction[index] = true;
        }
        this.SET_GOODS_ORDER_BY(this.tempSort.title);
        this.SET_GOODS_ORDER_SORT_BY(this.tempSort.sortBy);
      }
    },
    created() {
      if (this.$route.query.categoryId !== undefined) {
        ...
        this.searchGoodsList(this.$route.query.sreachData);
      }
    },
    mounted() {
    ...
    watch: {
      '$route'(to, from) {
        if (to.query.categoryId !== undefined) {
          this.loadGoodsList(to.query.categoryId);
        } else if (to.query.sreachData !== undefined) {
          this.searchGoodsList(to.query.sreachData);
          this.searchItem = to.query.sreachData;
        }
      }
    },
    components: {
