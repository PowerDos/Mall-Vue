...
public class OrderService {
  ...
  public PayTransactionDO generatePrePayTransaction(PayInfoDTOInput payInfo, String userId) throws Exception {
    ...
    List<HandleGoodStorage> bugInfos = new ArrayList<>();
    for (OrderDetailDO orderDetailDO : payList) {
      orderDetailDO.setPrice(orderDetailDO.getPrice() * 100L);
      HandleGoodStorage bugInfo = new HandleGoodStorage();
      bugInfo.setSpecsId(orderDetailDO.getSpecsId());
      bugInfo.setCount(orderDetailDO.getCount());
      bugInfos.add(bugInfo);
    }
    // 修改example_product_specs库存
    // TODO 在这里会存在分布式事务的问题
    BaseResponseStruct<JSONObject> handleStorage = goodServiceFeign.handleGoodStorage(bugInfos);
    if (handleStorage.getRtnCode().equals(Constants.HTTP_RES_CODE_500)) {
      // 删除库存失败！
      throw new Exception(handleStorage.getMsg());
    }
